function cccSendInternalEmail_(b, i) {
  GmailApp.sendEmail(
    ...
    {
      htmlBody: cccBuildInternalEmail_(b, i),
      name: "Cost Control Consulting",
      replyTo: b.email
    }
  );
}
...
function cccSendClientEmail_(b, i) {
  GmailApp.sendEmail(
    ...
    {
      htmlBody: cccBuildClientEmail_(b, i),
      name: "Cost Control Consulting",
      replyTo: CCC.OWNER_EMAIL
    }
  );
}
    ...
    '<div style="margin:0;padding:24px;background:#eef7fd;font-family:Arial;color:#17324d">' +
      '<div style="max-width:760px;margin:auto;background:#fff;border-radius:14px;overflow:hidden;border:1px solid #cfe5f5">' +
        ...
        '<div style="background:#9fd3f4;padding:24px 30px;border-bottom:5px solid #1f5d8f">' +
          '<div style="font-size:27px;font-weight:700;color:#123f60">Cost Control Consulting</div>' +
          '<div>Clarity. Confidence. Compliance.</div>' +
        '</div>' +
...
function cccBuildInternalEmail_(b, i) {
   ...
   * APPOINTMENT DETAILS
   */
  rows += section("Appointment Details");
   ...
   */
  rows += section("Business Information");
   ...
   * ACCOUNTING / SYSTEMS
   */
  rows += section("Accounting and Business Systems");
   ...
   */
  rows += section("Consultation Details");
  ...
  rows += row(
    "Business Challenges",
    Array.isArray(b.businessChallenges)
   ...
   */
  rows += section("Consent and System Information");

  rows += row(
    "Consent to Contact",
    yn(b.consentToContact)
  );
  ...
  var html =
    '<div style="margin-bottom:18px;padding:14px 16px;background:#eaf6fd;border-left:5px solid #1f5d8f;border-radius:8px">' +
      '<div style="font-size:14px;color:#50697d">CCC Reference Number</div>' +
    ...
    '<div style="margin-top:22px;padding:14px;background:#fff8e8;border:1px solid #ead7a4;border-radius:8px">' +
      '<strong>Internal Use:</strong> This email contains the complete consultation information submitted by the client for review and preparation before the appointment.' +
    '</div>';
...
function cccBuildClientEmail_(b, i) {
  ...
  return cccEmailShell_(
    ...
    '<p>Thank you for contacting Cost Control Consulting.</p>' +
    ...
    '<p>We will review your information before the meeting so the consultation can be focused and productive.</p>'
  );
}
